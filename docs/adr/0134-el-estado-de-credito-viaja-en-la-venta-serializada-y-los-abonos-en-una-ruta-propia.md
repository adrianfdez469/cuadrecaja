# ADR 0134: El estado de crédito viaja en la venta serializada, y los abonos en una ruta propia
...
El criterio 1 de F-037 pide distinguir en `/ventas` una venta a crédito **con saldo vivo** de una
...
- `IVenta` (`src/schemas/venta.ts`) ya declara `creditoBase`, `clienteId` y `clienteNombre`, y la
  GET que alimenta la lista (`api/venta/[tiendaId]/[cierreId]/route.ts`, de **F-034**, cerrada) ya
  ...
  pueden pintar.
- Esa GET no está en el patrón `api/venta/**/[ventaId]/**` que F-037 tiene concedido (dosier § 9).
- `GET /api/cuentas-por-cobrar` (F-035) sí devuelve todas las cuentas con `ventaId`,
  `saldoPendiente` y `settledAt`, pero exige el permiso `recuperaciones.cuentasporcobrar.acceder`,
  que un cajero no tiene por qué tener. **Ningún criterio de F-037 pide un permiso nuevo.**
...
y la GET de F-034 se extiende por delegación escrita.** Los abonos, que son una lista y solo hacen
falta al abrir el detalle, viajan por una **ruta propia de F-037** bajo `[ventaId]`.

Tres piezas:

1. **`ventaCreditoResumenSchema`** — un objeto nuevo (`src/schemas/ventaCredito.ts`, de F-037) con
   `cuentaId`, `montoOriginal`, `saldoPendiente`, `settledAt`, y el resumen de cobros (`cobros`,
   `cobrosMontoBase`) y de filas del libro (`movimientos`) que el gate del ADR 0133 necesita.
...
2. **Delegación escrita** sobre cuatro archivos ajenos, todos de forma aditiva: `src/schemas/venta.ts`
   y la GET del listado (F-031/F-034), más `src/store/salesStore.ts` y
   `src/app/pos/components/SalesDrawer.tsx` (F-034), que son los que hacen llegar el bloque al
   `Sale` de los dos drawers del POS donde el criterio 5 exige el bloqueo. El precedente es el
   propio F-034, que editó dos archivos de F-031 por delegación escrita (ADR 0118).
3. **`GET /api/venta/[tiendaId]/[cierreId]/[ventaId]/credito`** — ruta nueva **dentro** del patrón
   concedido a F-037, con el mismo régimen que su hermana del listado: `assertTiendaTenant` con
...
la bandeja de pedidos online de F-038 usa el mismo (dosier § 4: «No se escriben dos»).
...
El `ui-designer` reabrió esta mitad de la decisión (`.agents/designs/F-037.md`, § 8, pregunta 1) con
...
| Un endpoint propio de F-037 que devuelva el mapa `ventaId → estado` para el período | Dos peticiones para pintar una lista, dos estados de carga y una ventana en la que el chip no coincide con la fila. Y sigue sin ser un campo de la venta |
...
| Ampliar la excepción a `SaleExtrasSummary.tsx` y pintar el crédito ahí | Arrastraría el libro de abonos al `Sale` persistido del POS y pondría el layout de dos pantallas del POS en manos de F-037 |
...
- El chip queda listo para F-038 sin un segundo componente.

**En contra / coste asumido:**
- Cuatro archivos ajenos editados por delegación. Cada uno es aditivo y está nombrado en el
  contrato, pero es superficie de conflicto si F-034 se reabriera.
