# ADR 0106: Los selectores de cliente nacen montados en la pantalla de configuración, no en el POS
...
F-031 entrega tres artefactos que el resto del epic va a consumir: el hook `useClienteSearch`, el
bottom sheet `ClienteSheet` y el `ClienteAutocomplete`. Los tres viven en archivos que el mapa de
propiedad del dosier (`.agents/cuentas-por-cobrar.md`, § 9) asigna a F-031.

Pero **el sitio donde el POS elegiría un cliente no es de F-031**. El mismo § 9 asigna
`src/app/pos/**` a **F-032**, que es el feature que introduce la venta a crédito. F-031 no escribe
ahí.

Eso deja un hueco concreto, y hay que resolverlo antes del paso 5 porque dos criterios de
aceptación de F-031 (`.agents/features.json`) hablan del selector como si ya estuviera montado:
...
**F-031 no escribe ni una línea en `src/app/pos/**`. Los dos selectores se entregan montados en
...
1. `ClienteSheet` y `ClienteAutocomplete` son las **dos superficies de búsqueda de la pantalla de
   ...
   en `.agents/designs/F-031.md`; que las dos sean alcanzables desde esa pantalla, no.
2. Sus props quedan fijadas en el contrato de interfaces de F-031 y **F-032 las monta en el
   checkout sin cambiarlas**. Si F-032 necesita una prop más, la añade F-032 y lo anota; F-031 no
   la reserva por adelantado.
3. El refresco en segundo plano del caché se entrega como una función exportada con firma fija
   (`refreshClientesCache`), **sin llamador en F-031 dentro del POS**. F-032 la engancha donde ya
   se agenda `syncPendingSales`, respetando `shouldDeferPosBackgroundOperations`. Es el mismo
   patrón de «firma reservada» que F-029 usó con `applyMovimientoCuentaPorCobrar`.

## Alternativas consideradas

| Opción | Por qué no |
|--------|-----------|
| Que F-031 monte el selector en el checkout del POS | Sería un control **muerto en producción** entre la entrega de F-031 y la de F-032: un selector que elige un deudor para una venta que todavía no puede ser a crédito. Y rompe el mapa de propiedad del § 9 sobre el archivo más disputado del epic (`src/app/pos/page.tsx`, donde F-032 tiene que arreglar además los bugs 1 y 2 del dosier § 4) |
...
| Aplazar los dos componentes a F-032 y dejar en F-031 solo el CRUD y el hook | Deja los criterios 8, 9 y 10 sin nada que ejercitar —el caché existe para el selector— y convierte a F-032 en un feature con dos superficies visuales nuevas encima de la venta a crédito |
| Montar el `ClienteAutocomplete` en `PedidoEntregaDialog` | Ese archivo vive en `src/components/tiendaOnline/**`, que el § 9 asigna a **F-036** |
...
- F-032 recibe dos componentes ya ejercitados y una firma de refresco que no tiene que inventar.
...
- Hasta que F-032 entregue, elegir un cliente en la pantalla de configuración no tiene efecto sobre
