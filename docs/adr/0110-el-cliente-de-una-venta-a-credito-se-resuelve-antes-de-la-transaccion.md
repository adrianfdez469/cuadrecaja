# ADR 0110: El cliente de una venta a crédito se resuelve antes de la transacción, y el cliente nuevo nace dentro de ella

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-032 (vender a crédito en el POS)

> Este ADR cierra las **tres preguntas abiertas** del spec de F-032 que tocan al servidor: el orden
...
>
> El contrato de interfaces de F-032 (`.agents/specs/F-032.md`, § 5) traduce esta decisión a firmas
...
- `checkCreditInvariant` (`src/lib/cuentasPorCobrar/creditInvariant.ts`, entregado por F-029 y
...
- El contrato de F-029 fija además un orden que F-032 hereda: `reconcileSaleTotal` corre
...
- `createOrReactivateCliente` (`src/lib/clientes/clienteUpsert.ts`, F-031) resuelve el alta con
  lectura previa y un reintento, pero habla con el `prisma` global y **no abre transacción**
  (ADR 0107). Su función pura `decideClienteUpsert` sí es reutilizable tal cual.
...
3. `checkCreditInvariant` recibe ese `clienteId` y **no cambia de firma ni de comportamiento**. En
   ...
   `CREDIT_INVARIANT_HTTP_STATUS`, que también es de F-029.
...
| Opción | Por qué no |
|--------|-----------|
| Dar a `checkCreditInvariant` un tercer dato de entrada (`clienteNombre`) | Es un archivo de F-029, cerrado con `passes: true`, con sus doce criterios verificados contra la firma actual. Y no arregla nada: la función seguiría sin poder decir si ese nombre corresponde a un cliente **del negocio**, que es la otra mitad de la regla dura. Cambiar una firma cerrada para que siga sin contestar la pregunta es coste sin beneficio |
...
| Más de un reintento | El único fallo que un reintento puede resolver es «alguien ganó la carrera», y eso se resuelve a la primera o no era eso (E-038, y el mismo razonamiento que `CLIENTES_UPSERT_RETRIES` en ADR 0107) |
...
| Llamar a `checkCreditInvariant` también con `creditoBase = 0` | La función es **más estricta que lo que el servidor hace hoy**, y F-029 lo dejó escrito: con `creditoBase = 0` el único desenlace posible además de `ok` es `TOTAL_MISMATCH`, que **sí** es alcanzable hoy —una venta con un sobrepago que no quedó registrado como vuelto se persiste sin más—. Sin la guarda, ventas al contado que hoy se registran empezarían a devolver 400, que es exactamente la degradación que el criterio 1 de F-032 prohíbe |
...
- `checkCreditInvariant`, `CREDIT_INVARIANT_VIOLATIONS` y `CREDIT_INVARIANT_HTTP_STATUS` se
  consumen **tal cual**: F-029 no se reabre, y las dos mitades de la primera regla dura —«presente»
...
- **Un reintento es más caro que reintentar solo el `create`**, porque en Postgres eso último no
  ...
  > **Enmienda del 2026-09-09** (M1 de `.agents/F-032-seguridad.md`). Este párrafo decía antes «se
...
- **Dos ventas offline con nombres parecidos pero no iguales siguen creando dos clientes.**
  ...
  (F-031, ADR 0107): «Ana Pérez» y «ana perez» son dos filas. Fusionar clientes es deuda conocida
  de F-031 y sigue sin resolverse aquí.
...
- **Aislamiento multi-tenant.** Las dos lecturas y las dos escrituras de `Cliente` van acotadas con
  ...
  que es el invariante que F-029 escribió en el comentario de esa columna: es la única vía por la
...
- **Permisos.** Esta decisión no cambia ninguna puerta; ver **ADR 0112**.
...
- **Reversión.** Reversible salvo por las filas escritas. Quitar el código deja `Venta.creditoBase`
  ...
  la caja iterando `pagosDetalle`, que es por qué la caja cuadra sola (ADR 0104). No hace falta
