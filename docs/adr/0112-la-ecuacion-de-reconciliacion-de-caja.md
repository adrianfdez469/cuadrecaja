# ADR 0112: La ecuación de reconciliación de caja

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-032 (y gobierna la lectura del cierre para todo el epic de cuentas por cobrar)
...
> usarlo. El único que conviene tener a mano es el [ADR 0111](0111-el-credito-es-una-columna-no-una-linea-de-pago.md),
...
- Como el crédito no es una línea de `pagosDetalle` (ADR 0111), **una venta a crédito no aporta
...
`where`**, no solo aquí, y F-032 lleva su propio criterio de aceptación cuyo único propósito es
atrapar esto.
...
moneda base y no en divisa (ADR 0111).
...
- **Aislamiento multi-tenant.** Las consultas de deuda que alimentan la ecuación se acotan a **una
  ...
  las consultas. F-032 lleva un criterio propio que lo comprueba **ejecutándolo**: sembrar una
