# ADR 0115: El enum de la base dice `CONDONACION` y la cara visible dice «perdonar deuda»
...
`AJUSTE_DEVOLUCION`, `CONDONACION`, `REVERSION_ABONO`—, migrado y verificado por **F-029**, que
...
F-033 es el primer feature que **escribe** uno de esos valores desde una pantalla, y al llegar a la
...
un feature ya cerrado, con su `prisma generate`, su reverificación de F-029 y F-030 —que ya leen
...
| Opción | Por qué no |
|--------|-----------|
| **Renombrar el enum a `PERDON` / `PERDON_DEUDA`** | Migración de un tipo enumerado de PostgreSQL sobre F-029, que está en `passes: true` y cuyo criterio 1 se verificó midiendo `\d` sobre las tablas antes y después. Arrastra a F-030, que ya filtra por esos literales en `periodCollectionsWhere`, y a `MOVIMIENTO_CUENTA_POR_COBRAR_SIGN`. Coste alto, ganancia cero para el usuario: nadie ve el valor del enum |
...
- F-029 no se reabre y su `passes: true` sigue significando lo que significaba.
