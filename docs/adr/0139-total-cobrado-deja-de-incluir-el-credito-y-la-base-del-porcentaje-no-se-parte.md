# ADR 0139: «Total cobrado» deja de incluir el crédito, y la base del porcentaje no se parte en dos

**Estado:** aceptado
**Fecha:** 2026-09-10
**Feature:** F-039 (el crédito en los reportes de operación y rentabilidad)
**Se apoya en:** [ADR 0138](0138-el-credito-es-un-campo-obligatorio-de-la-venta-normalizada-y-se-comprueba-antes-de-la-rama-legado.md) ·
[ADR 0111](0111-el-credito-es-una-columna-no-una-linea-de-pago.md) ·
[ADR 0132](0132-la-nota-del-credito-no-reutiliza-note-y-los-criterios-se-anclan-en-data-testid.md) ·
...
En cuanto el ADR 0138 mete la fila `credito` en el mix, los dos papeles dejan de coincidir. El
criterio 2 de F-039 **exige** que los porcentajes se calculen sobre una base que incluya el crédito
...
cierre —y que el ADR 0129 resolvió allí explicando la cifra en vez de corregirla—, pero aquí no hay
nada que explicar: el nombre de la tarjeta es una afirmación aritmética sobre lo que hay en la
gaveta, y sería falsa.

Ninguno de los ocho criterios de aceptación de F-039 obliga a arreglarlo. Esa es precisamente la
...
- `totalBase` no cambia de definición ni de valor respecto a lo que el ADR 0138 establece: todas las
...
- Se añade una tarjeta de crédito, condicionada a que el período tenga crédito
  (`hasCreditKpi`, media centésima — el mismo umbral que F-036 ya definió y que se **importa**, no
...
reconstrucciones legado desde `totalcash`/`totaltransfer`, y la fila de crédito del ADR 0138. Los
...
- Las afirmaciones de los criterios sobre el DOM se anclan en `data-testid` y no en copy, que el
  `ui-designer` todavía no ha escrito (ADR 0132, E-011).
