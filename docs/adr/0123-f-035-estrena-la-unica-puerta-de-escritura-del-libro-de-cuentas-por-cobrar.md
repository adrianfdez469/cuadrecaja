# ADR 0123: F-035 estrena la única puerta de escritura del libro de cuentas por cobrar

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-035 (panel de cuentas por cobrar y registro de cobros)

> Concede a F-035 la propiedad de **dos archivos nuevos** dentro de `src/lib/cuentasPorCobrar/`, una
> carpeta cuyo mapa de propiedad enumeraba cuatro archivos y no la cerraba entera. Es la misma
> figura del **ADR 0118** (delegación escrita entre features del epic) y del **ADR 0121** (un
...
Esa firma existe, pero solo como firma. El contrato de **F-031** la fijó en su § 9 —nombre de
...
firmar código que no puede ejecutar. **F-034** declinó por lo mismo: crea la `CuentaPorCobrar` al
vender, pero no escribe ni un movimiento, «ese libro lo estrena el primer abono, que es de F-035».

El mapa de propiedad del dosier (§ 9) enumera para F-031 exactamente
`src/lib/cuentasPorCobrar/{saldo,aging,refundSplit,creditInvariant}.ts` y **no dice nada de la
carpeta**. Para F-035 enumera `src/app/cuentas-por-cobrar/**`, `api/cuentas-por-cobrar/**`,
...
`src/app/cuentas-por-cobrar/utils/` —territorio propio de F-035— obligaría a la ruta de la API a
...
**F-035 es dueño de dos archivos nuevos dentro de `src/lib/cuentasPorCobrar/`:**

1. **`src/lib/cuentasPorCobrar/applyMovimiento.ts`** — la puerta única de escritura del libro. El
   nombre **no se inventa aquí**: es el que el contrato de F-031 § 9 ya reservó, y se adopta
...
`creditCustomer.ts` son de F-031 y se **consumen** sin cambiarles una firma.

Por la misma razón y con el mismo alcance, F-035 crea `src/schemas/cuentasPorCobrarPanel.ts` (los
...
mismo epic: **F-033 creó `src/schemas/clienteSaldo.ts` y `src/lib/clientes/clienteSaldo.ts`** sin que
...
| Opción | Por qué no |
|--------|-----------|
| **Que F-031 lo hubiera implementado** | Ya se decidió que no, y el argumento sigue en pie: ninguno de sus criterios lo ejercita, así que su `qa` habría tenido que firmar código sin ejecutarlo. La decisión no se revisa; solo se completa nombrando al sucesor |
| **Que la lógica viva dentro de la route handler del abono, sin archivo propio** | Rompe la propiedad central del epic. `saldoPendiente` es denormalizado, y en cuanto haya dos rutas que escriban el libro —el abono, el perdón, la reversión, y mañana el `AJUSTE_DEVOLUCION` de F-037— hay cuatro sitios donde recalcularlo mal. El helper existe precisamente para que sean cero |
| **Poner la agregación del panel en `src/app/cuentas-por-cobrar/utils/` y que la ruta importe de ahí** | Sería territorio propio de F-035 y no necesitaría esta concesión, pero estrenaría un import `@/app/**` desde una route handler que **ningún** archivo del repositorio hace hoy. Inventar una dirección de dependencia nueva para ahorrar un párrafo de ADR es mal negocio |
...
| **Abrir un feature aparte solo para el helper** | Mete una dependencia entre features en el cierre de un epic de nueve, para un archivo que solo F-035 va a ejercitar. Y deja la pregunta de fondo —quién lo posee— igual de abierta que estaba |
...
- La firma queda **exactamente** donde F-031 dijo que iba a estar, con el nombre que reservó: quien
  siga la pista desde el contrato de F-031 encuentra el archivo.
...
- F-037, cuando escriba su `AJUSTE_DEVOLUCION`, hereda una puerta ya construida y verificada en vez
  de inventar la segunda.

**En contra / coste asumido:**

- **F-035 escribe en una carpeta que el mapa de propiedad asociaba a F-031.** El riesgo de colisión
  es real aunque los archivos sean nuevos: si F-031 tuviera que reabrirse por otra razón, dos
...
- **La concesión del coordinador hablaba de UN archivo nuevo; este ADR otorga dos.** No es una
  ...
  dueño y F-035 es el primero que la necesita—, pero **es una decisión que el coordinador tiene que
...
- **La regla se puede leer de más.** «Lo crea quien lo necesita» vale para un archivo que **no
  ...
  (ADR 0118) o un defecto verificado que bloquee un criterio (ADR 0121).
