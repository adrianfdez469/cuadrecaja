# ADR 0104: El crédito es una columna, no una línea de pago

**Estado:** aceptado
**Fecha:** 2026-09-08
**Feature:** F-029 (y gobierna todo el epic de cuentas por cobrar, F-029 … F-037)
...
>
> El contrato de interfaces de F-029 (`.agents/specs/F-029.md`) traduce esta decisión a firmas
...
1. **Dos agregadores de caja tragarían el tercer valor en silencio, por partida doble.**
   ...
   F-029 (criterio 4) porque cierra la trampa para cualquier forma de pago futura, no solo para el
   crédito.
...
está bien, sino para **explicarlo**. La ecuación completa de reconciliación es del ADR de F-030.
...
cablea **F-032**. F-029 solo entrega la función pura que las codifica
...
- El endurecimiento del `else` de los dos agregadores (F-029, criterio 4) **sobrevive aunque el epic
...
- **Una columna más en `Venta`**, la tabla más leída del sistema, y un campo más que propagar por
  ...
  existe. Es trabajo de F-032 y está enumerado en el dosier § 6.
- **El chequeo de la invariante es más estricto que lo que el servidor hace hoy**, porque hoy no
  ...
  En F-029 no cambia nada, porque ninguna ruta llama todavía a la función; **F-032 tiene que
...
- `saldoPendiente` es **denormalizado** y puede derivar si alguien escribe en
  ...
  para reparar. F-029 fija sus nombres y **no los implementa**: los escribe el primer feature que
...
- **El salto directo tiene un invariante que el schema no impone.**
  ...
  crea la fila —F-032, en la misma `$transaction` que la venta— deriva ese valor de la **misma
...
- **Borrar en blando a un deudor con deuda viva se bloquea.** `Cliente.deletedAt` es un `UPDATE`:
  ...
  dinero que el negocio cree cobrado. La regla: el DELETE de `api/clientes/**` (F-031) devuelve
...
- **Autorización.** Ninguna de las reglas duras se valida solo en el cliente: la función pura vive
  en `src/lib/` y F-032 la invoca desde las **dos** rutas de venta. La mitad de la primera regla que
...
- **Escalabilidad.** `CuentaPorCobrar` nace con tres índices para los tres accesos que el epic ya
  ...
  sobre una tabla sin filas no dice nada (E-023): quien tenga filas reales —F-030 o F-033— es quien
  puede contrastarlos y ajustarlos.
- F-029 **no añade ningún índice a `Venta`**. Un `CREATE INDEX` normal sobre una de las tablas más
  ...
  consulta.
- El filtro de cuentas abiertas al corte que usará F-030 es
  ...
  comentario va junto al `where`, y F-030 lleva su propio criterio para atraparlo.
