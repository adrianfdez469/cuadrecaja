# ADR 0126: La tercera razón de bloqueo son los cobros ya recibidos, no la deuda viva

**Estado:** aceptado
**Fecha:** 2026-09-10
**Feature:** F-035

> Este ADR resuelve una **tensión entre dos criterios de aceptación vinculantes de F-035** y fija
...
El dosier del epic (`.agents/cuentas-por-cobrar.md` § 4) y el campo `notes` de F-035 en
...
del `spec` (`.agents/specs/F-035.md`, criterio 5, «Protocolo»), que es elaboración del paso 3, no
texto del backlog.
...
F-032 no existía ninguna venta con cero líneas de pago.
...
criterio de F-035 exige, y por eso está **nombrada, con su propio motivo y en la lista de
...
- El front necesita saber si la cuenta tiene cobros, y eso obliga a que el dato viaje en la venta
  serializada y en el `Sale` del POS (ADR 0127). Un `Sale` local sin sincronizar no trae el bloque
