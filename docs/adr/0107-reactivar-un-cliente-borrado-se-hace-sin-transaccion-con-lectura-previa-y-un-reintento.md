# ADR 0107: Reactivar un cliente borrado se hace sin transacción, con lectura previa y un solo reintento del `P2002`
...
`Cliente` lleva `@@unique([nombre, negocioId])` y soft delete (`deletedAt`). F-029 dejó escrita la
consecuencia y a quién le tocaba resolverla (`.agents/specs/F-029.md`, § 2.1):
...
> de recrear— es de **F-031**.

El criterio 4 de F-031 lo exige: crear un cliente con el nombre de uno ya borrado lo **reactiva**
...
conexión de F-032** (dosier § 11, ficha E-043): si el POST falla con 409 en vez de reactivar,
F-032 no tiene sobre qué apoyarse, y la salida obvia —abrir una segunda columna `@unique` global
...
borrado en blando decidido en `.agents/specs/F-029.md`, § 2.1.
...
| Excluir las filas borradas del índice único (índice parcial) | Prisma no lo expresa en el schema, obligaría a SQL a mano en la migración, y **destruiría el eje de idempotencia que F-032 necesita** (dosier § 11, E-043): dos filas con el mismo nombre podrían coexistir si una está borrada |
...
| Reactivar también desde el `PUT` (renombrar un cliente al nombre de uno borrado lo fusiona) | Fusionar dos clientes está declarado fuera de alcance en las `notes` de F-031. El `PUT` responde 409 en ese caso |
...
- F-032 tiene un eje de idempotencia estable para el alta offline, y es el que ya existe.
