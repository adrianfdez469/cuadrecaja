# ADR 0109: El permiso de clientes va a la plantilla `vendedor`, no a `almacenero`
...
F-031 añade dos permisos a `src/constants/permisos/permisos.json`:
...
- `recuperaciones.cuentasporcobrar.acceder` — ver la entrada de menú «Cuentas por Cobrar» y, cuando
  F-033 la construya, el panel al que apunta.

`src/constants/permisos/permisos.templates.ts` reparte permisos por plantilla de rol (`vendedor`,
`almacenero`, `administrador`). El spec de F-031 dejó esto explícitamente abierto para el
...
**en mitad de una venta a crédito en el POS** (F-032, y el criterio 10 de F-031 diseña el caso de
...
| Un tercer permiso propio para el alta rápida (`operaciones.clientes.crear`) | El spec y las `notes` de F-031 fijan **dos** permisos nuevos. Un tercero multiplica el cableado de E-047 y parte la regla «quien puede crear un cliente puede corregirlo» en dos configuraciones que se desincronizan |
| Dar a `vendedor` solo POST y no PUT/DELETE, con dos permisos | Mismo problema que la anterior, y el borrado ya es **reversible**: es un `deletedAt`, y ADR 0107 hace que volver a dar de alta el mismo nombre restaure la misma fila |

## Consecuencias

**A favor:**
- El alta rápida de cliente que F-032 necesita en el POS funciona con la plantilla que se usa en el
...
- Un usuario creado desde la plantilla `vendedor` ve la entrada «Clientes» del menú de
  ...
  blando y reversible (ADR 0107), la puerta de 409 impide desactivar a un cliente con deuda viva
  (`.agents/specs/F-029.md`, § 2.1), y un administrador puede quitarle el permiso a ese usuario en
  cualquier momento.
...
Levantado por `.agents/F-031-seguridad.md`, hallazgo 🔴-1. La frase de la sección **Decisión**
...
persona antes de fiarle más (ADR 0108), y el criterio 8 del backlog ya exige que ese campo llegue al
caché del selector.

**La frontera, entonces:** la frase de arriba se aplica al **panel agregado** de F-033 —quién debe,
cuánto, desde cuándo, el histórico de abonos—, que sí va detrás de
`recuperaciones.cuentasporcobrar.acceder`. **No** se aplica al saldo por cliente que F-031 expone
sin permiso. F-032, F-033, F-035 y F-037 no deben tratar el saldo de un cliente como un dato
