# ADR 0111: F-032 edita `src/schemas/pago.ts` y `src/schemas/venta.ts` por delegación escrita de F-029

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-032 (vender a crédito en el POS)

> Cierra la primera pregunta abierta del spec de F-032: **quién edita un archivo de un feature ya
> cerrado con `passes: true`**, y **cómo** se edita para que lo que aquel feature verificó siga
> siendo cierto.

## Contexto

El mapa de propiedad del dosier del epic (`.agents/cuentas-por-cobrar.md`, § 9) da a **F-029**
«`src/schemas/cliente.ts`, `cuentaPorCobrar.ts` y las extensiones de `venta.ts`/`pago.ts`», y a
**F-032** «`src/app/pos/**`, las dos rutas de venta, `src/store/salesStore.ts`,
...
F-029 está **cerrado con `passes: true`**. Y sin embargo F-032 necesita tocar dos de sus archivos:
...
El dosier § 6 asigna los dos **comportamientos** a F-032, pero no dice quién edita los archivos, y
el contrato de F-031 —el otro feature que consume estos schemas— dice explícitamente «Ningún archivo
de F-029 se edita en F-031», lo que podría leerse como una regla general.

Hay dos precedentes que apuntan en direcciones distintas:

- El `arch-guardian` de **F-029** añadió `src/constants/pago.ts`, un archivo que su propio spec no
  enumeraba, porque la necesidad apareció al diseñar y no al planear.
- El `arch-guardian` de **F-031** decidió lo contrario para `src/schemas/cliente.ts`: creó
  `src/schemas/clienteSaldo.ts` aparte en vez de extender el archivo cerrado.

Y hay un dato que decide: el propio contrato de F-029, en su § 3.5, **ya cedió por escrito** el
primero de los dos cambios:

> `pagosDetalleAppSchema` **no cambia en F-029**. El `superRefine` que permite una venta 100 % a
> crédito con `pagosDetalle` vacío es de **F-032** (dosier § 6, «Rutas de venta»), que es dueño de
> las dos rutas de venta.

## Decisión

**F-032 edita `src/schemas/pago.ts` y `src/schemas/venta.ts` directamente, con ediciones
estrictamente aditivas, y crea `src/constants/creditoVenta.ts` para todo lo demás.**

> **Enmienda del 2026-09-09**, al cerrar el hallazgo H1 de `.agents/F-032-seguridad.md`. La
> delegación se amplía a un tercer archivo, **`src/schemas/cliente.ts`** —de **F-031**, cerrado—,
> para añadir la cota de juego de caracteres a `createClienteSchema.nombre`, y se crea
> `src/utils/printableText.ts`. La razón es la del **ADR 0113**: `POST /api/clientes` es el **otro**
> escritor de `Cliente.nombre`, y cerrar una de las dos puertas y no la otra no arregla nada. La
> edición es una sola línea, restrictiva sobre una entrada, y ningún criterio de F-031 usa nombres
...
> que abre el riesgo**, y F-031 no podía haberla tomado — su `Cliente.nombre` no se imprimía.

Las tres reglas que hacen que eso sea seguro:

1. **Nada de lo que F-029 verificó cambia de comportamiento.** `pagoLineaSchema` se queda idéntico
   —el criterio 12 de F-029 existe precisamente para que nadie le añada un tercer valor a `tipo`— y
...
2. **Cada campo nuevo es opcional y se inserta antes de la línea-ancla, nunca sustituyéndola**
   ...
   en `ventaSchema`, y nada más — `creditoBase` y `clienteId` ya los dejó puestos F-029 en los dos.
...
| Espejar F-031 y crear `src/schemas/ventaCredito.ts` para `clienteNombre` de `IVenta` | Mismo problema: `IVenta` es lo que devuelve el GET y lo que consumen `reloadSales`, `ventaToSale` y `src/app/ventas/**`. Un tipo paralelo se desincroniza del real a la primera |
| Reabrir F-029 para que hiciera los tres cambios | Está cerrado con `passes: true` y su `qa` verificó doce criterios contra ese árbol. Reabrirlo invalidaría esa firma para arreglar algo que su propio contrato ya había delegado |
...
| Reutilizar `src/constants/pago.ts` (F-029) para las constantes nuevas | Es otro archivo cerrado y su contenido es de otro dominio: el aviso de una línea de pago desconocida. Un archivo nuevo no colisiona con nada |
...
- **La cota de `clienteNombre` está escrita dos veces**: `max(200)` en `multimonedaExtrasSchema` y
  ...
  del contrato de F-029 prohíbe la arista `pago.ts → cliente.ts` para evitar un ciclo. La
  duplicación se convierte en algo que la suite atrapa: un test de la lista de testabilidad de F-032
