# ADR 0120: Una reversión de abono no lleva líneas de pago

**Estado:** aceptado · **enmendado el 2026-09-09 por el [ADR 0121](0121-la-reversion-de-un-abono-se-descuenta-de-la-caja-como-un-espejo-en-negativo.md)**
**Fecha:** 2026-09-09
...
>
> F-033 estrena el primer escritor de `REVERSION_ABONO`, y los dos motores que leen el libro
...
Ambos archivos son de **F-030**, cerrado. F-033 no los toca — el dosier § 9 se lo asigna a otro
...
Arreglarlo es de F-030 o de quien herede esos archivos. Lo que F-033 sí decide es **qué forma tiene
...
> **Deuda conocida, dueño F-030 (o quien herede `src/lib/cierre/**` y `src/lib/movimiento/caja.ts`).**
> `periodCollectionsWhere` y la carga de abonos de `construirResumenCajaAbierta` filtran
> `tipo: "ABONO"` con el motivo escrito «REVERSION_ABONO has no writer yet». F-033 estrena ese
...
Esa nota va, con estas palabras, en el docstring de `applyMovimiento.ts` y en el contrato de F-033.
No se escribe en los archivos de F-030: son de otro feature.
...
| **Que F-033 arregle también `caja.ts` y `loadCierreInput.ts`** | Son de F-030, que está cerrado y verificado. El ADR 0114 permite tocar lo ajeno cuando hay un defecto **verificado ejecutando** que **bloquea un criterio**: ninguno de los trece criterios de F-033 mide la caja tras una reversión —el criterio 11 mide que **perdonar** no la toca, que es otra cosa—, así que la condición no se cumple. Se declara la deuda y se le pone dueño |
...
- **La deuda se declara en un ADR y en un docstring, no en un feature abierto.** El ADR 0114 cuenta
  exactamente cómo termina eso: en F-031 se difirió un defecto «para quien lo posea», no había
  quien lo poseyera, y volvió doce días después. Aquí el dueño está nombrado —F-030 y sus
...
**Qué cambió fuera de este ADR.** El humano decidió que F-033 **arregle** el desajuste, por
delegación sobre los dos puntos de F-030 que filtran `tipo: "ABONO"`. Eso está razonado en el
[ADR 0121](0121-la-reversion-de-un-abono-se-descuenta-de-la-caja-como-un-espejo-en-negativo.md).
...
> resta**: `pagoLineaSchema` (`src/schemas/pago.ts`, de F-029) declara `monto: z.number().positive()`
...
- La sección «Decisión», donde dice *«el desajuste con los motores de caja se declara, no se
  ...
  el § 13, que enumera los criterios de F-030 que hay que re-ejecutar.
