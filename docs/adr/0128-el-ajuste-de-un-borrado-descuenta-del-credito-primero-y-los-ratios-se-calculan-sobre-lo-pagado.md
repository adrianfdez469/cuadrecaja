# ADR 0128: El ajuste de un borrado descuenta del crédito primero, y los ratios se calculan sobre lo pagado
...
3. **La deuda no se escribe a mano.** `nuevoCredito` se persiste en `Venta.creditoBase`, y la bajada
   ...
   `applyMovimientoCuentaPorCobrar` —la única puerta de escritura (F-029)— dentro de la **misma**
...
| Bloquear el borrado de productos en toda venta a crédito y no repartir nada | Es lo que el ADR 0126 descarta: hace inalcanzable el criterio 8 |
