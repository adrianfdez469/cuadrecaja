# ADR 0108: El caché de clientes es una proyección de cuatro campos, acotada, que se descarta entera al cambiar de versión
...
- Las `notes` de F-031 en `.agents/features.json`: «un negocio con 5.000 clientes no debe mandarlos
  a `localStorage`».
- Los criterios 8 y 9 de F-031: lo guardado no supera `CLIENTES_CACHE_SIZE`, contiene `id`,
...
- El caso que sí hay que acotar es **cambiar de negocio** con el mismo usuario. Se acota con dos
  ...
  la cierra el servidor: la regla dura de F-032 rechaza con 409 una venta a crédito cuyo
...
Levantado por `.agents/F-031-seguridad.md`, hallazgo 🔴-2, y aceptado. La versión original de este
...
copiar, `cartStore.ts` incluido. La limpieza de `cartStore` queda **fuera del alcance de F-031**,
anotada como deuda.
