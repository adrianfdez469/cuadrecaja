# ADR 0134: Las cifras de flujo del crédito en reportes **no** estrenan gate de permiso, y por qué eso hay que mirarlo

**Estado:** aceptado
**Fecha:** 2026-09-10
**Feature:** F-037 (el crédito en los reportes de operación y rentabilidad)
**Se apoya en:** [ADR 0123](0123-las-dos-cifras-de-flujo-entran-en-los-totales-almacenados.md) ·
[ADR 0109](0109-el-permiso-de-clientes-va-a-vendedor-y-no-a-almacenero.md) ·
...
F-034 mostró en el histórico de `/resumen_cierre` las dos cifras de flujo del crédito —
...
F-037 muestra **esas mismas dos cifras**, sumadas sobre los mismos cierres, en el estado de
...
`recuperaciones.reportes.rentabilidad`. Ninguno de los ocho criterios de aceptación de F-037 menciona
permisos.

Así que hay una pregunta que no se puede dejar sin responder: **¿debe F-037 replicar el gate de
F-034, o no?**
...
1. **Esconder la fila `credito` del mix rompería el criterio 2 en silencio, y solo para algunos
   usuarios.** La base de los porcentajes de la tabla incluye el crédito (ADR 0131). Un usuario sin
...
2. **Lo que el permiso protege es la cartera, no el total.** `recuperaciones.cuentasporcobrar.acceder`
   da acceso a quién debe, cuánto y desde cuándo. F-037 **no muestra ninguna identidad de deudor**:
...
- F-037 **no puede** añadir el nombre del cliente, el `clienteId`, la antigüedad de la deuda ni el
...
- F-037 **no lee `CierrePeriodo.totalPorCobrarAlCierre`** en ningún sitio: es el saldo de la cartera
...
que F-034 decidió esconderle en el histórico de cierres. Es una segunda puerta a la misma
...
visible. **No es una pregunta de arquitectura, es una pregunta de autorización**, y por eso F-037
dispara un paso de `security-guardian`: que se revise si la intención de F-034 era proteger el
...
- La divergencia con F-034 queda documentada y con dueño: la revisa `security-guardian`, no se
...
- **Aislamiento multi-tenant intacto y sin tocar.** F-037 no añade ninguna consulta: el crédito viaja
