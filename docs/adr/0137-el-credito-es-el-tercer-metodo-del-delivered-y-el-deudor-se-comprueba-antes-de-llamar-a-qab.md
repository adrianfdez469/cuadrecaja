# ADR 0137: El crédito es el tercer método del `DELIVERED`, y el deudor se comprueba —por negocio— **antes** de llamar a QAB
...
[ADR 0111](0111-el-credito-es-una-columna-no-una-linea-de-pago.md) ·
...
Entretanto, el epic de cuentas por cobrar (F-031 … F-039) construyó exactamente el vocabulario que
...
por cobrar**, y F-034 ya resolvió cómo se crea la deuda en la misma transacción que la venta, para
la ruta del POS.
...
(F-034) porque allí el cajero tiene el dinero delante y lo cuenta; aquí el encargado registra un
cobro que ocurrió —o no ocurrió— en otro sitio, y ningún criterio de F-038 ejercita una rama mixta.
...
que nadie toque el motor de caja**. No es una casualidad afortunada: es el ADR 0111 —el crédito
...
reservaron. **Ninguna aritmética las lee**: la deuda se denomina en moneda base (ADR 0111), y esa
...
- La caja del período **no se mueve** por un pedido entregado a crédito, y no hubo que tocar el
  ...
  en que se cobre, que puede ser otro — y ese comportamiento ya existía, de F-035.
...
- La deuda es **una fila por venta**, con `ventaId @unique` y los índices que F-031 ya puso sobre
  ...
  suma existen desde F-032 y F-035, y aquí solo se escribe.
