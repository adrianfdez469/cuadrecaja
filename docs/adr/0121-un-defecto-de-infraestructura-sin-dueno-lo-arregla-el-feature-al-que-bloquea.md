# ADR 0121: Un defecto de infraestructura sin dueño lo arregla el feature al que bloquea

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-034 (vender a crédito en el POS), y **transversal** en la regla que sienta

> Autoriza a F-034 a tocar `src/context/MessageContext.tsx`, con el alcance cerrado del § 6.9 de su
...
El `qa` rechazó F-034 en el ciclo 1 con 14/14 criterios de aceptación verificados ejecutando,
...
Falla **un** punto, el criterio de diseño 32 y la mitad del 31, y no está en ningún archivo de F-034:
...
**Es la segunda aparición.** El `qa` de **F-033** ya lo encontró, explorando fuera de sus criterios,
...
bloquea F-033, pero conviene una ficha de error para quien lo posea». **Nadie abrió la ficha, y
nadie era «quien lo posea», porque no hay quien lo posea.**

Y un dato que pesa más que los dos anteriores: el `ui-designer` de F-034, al escribir su contrato,
**leyó ese mismo archivo y concluyó que el hallazgo de F-033 ya no se reproducía en la versión
actual**. El `qa` lo ejecutó y sí se reproduce. **Leer no bastó** — ni para él, ni para el `qa` de
F-033, ni para quien lo mire la próxima vez.

Dónde bloquea ahora importa: el aviso que desaparece es el del **rechazo del nombre por caracteres
de control**, el estado que el ADR 0120 estrenó, y es la **única** explicación que el cajero recibe.
...
**F-034 arregla `src/context/MessageContext.tsx`, con un alcance cerrado y escrito por adelantado en
...
- La clase entera queda cerrada para todo el producto: cualquier error en ráfaga —los reintentos de
  sincronización, los 409 de F-033, lo que venga— vuelve a mostrar su `×N` en vez de desaparecer.
...
- La regla que sienta responde a la pregunta que F-033 dejó sin responder —«quien lo posea»— con
  algo verificable: lo posee, para este arreglo, quien lo necesita.

**En contra / coste asumido:**

- **F-034 toca infraestructura compartida en su cierre.** Es el riesgo real: `showMessage` lo llaman
...
- **Es la tercera delegación de este feature, y de una clase distinta.** Las de los ADR 0118 y 0120
  eran sobre archivos de **F-031 y F-033**, features del mismo epic con dueño identificable y
...
- **Seguridad, indirecta pero real.** El aviso que desaparecía es el del rechazo por caracteres de
  control (ADR 0120), y es la única realimentación del único control de esa clase que el cajero ve.
