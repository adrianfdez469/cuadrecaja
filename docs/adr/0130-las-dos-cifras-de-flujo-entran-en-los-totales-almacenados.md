# ADR 0130: Las dos cifras de flujo entran en los totales almacenados; el stock no gana fila
...
F-031 y el motor de F-032 las escribe en cada cierre y en cada recálculo
(`computeCierreTotals.ts:750-752`, persistidas por `persistCierreTotals`). Hasta ahora viajaban en
el JSON de las respuestas **sin tipo declarado**, porque `src/schemas/cierre.ts` es de F-036
(F-032 § 0.2).
...
- **Las tres en el schema, no dos.** El schema es el espejo de `CierreStoredTotals`, donde las tres
  ...
  trae las tres, F-032 § 6.3— como `after` —`computation.totals`— las llevan pobladas.
- **Dos filas en `ROWS`, no tres.** `totalPorCobrarAlCierre` es un **stock**: el saldo de la tienda
  ...
  recálculo ya la verifica F-032 (su criterio 9), que es donde vive el motor que la calcula.
- **Consecuencia buscada:** las dos filas nuevas se escriben añadiendo dos entradas al array. No
  ...
  genéricamente sobre `before[key]`/`after[key]`. El criterio 11 de F-036 sale de ahí.
...
| Añadir también `totalPorCobrarAlCierre` a `ROWS` | Un stock con un delta al lado se lee como un flujo. Su idempotencia es de F-032 y ya está verificada allí |
| Sumar `totalPorCobrarAlCierre` en el resumen | Cuenta la misma deuda N veces (dosier § 6, F-032 § 3.3.e) |
...
- Ninguna de las tres cifras estrena permiso: viajan en el JSON sin gate por la decisión ya cerrada
  de F-032 § 6.2.1, que este ADR no reabre.
