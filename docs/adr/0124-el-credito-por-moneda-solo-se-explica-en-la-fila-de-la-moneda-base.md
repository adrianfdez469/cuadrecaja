# ADR 0124: El crédito por moneda solo se explica en la fila de la moneda base
...
`prisma/schema.prisma` **no son de F-034**, y `ResumenMonedaCierre` es una tabla, así que un
...
| Opción | Por qué no |
|--------|-----------|
| Añadir el desglose por moneda a `ResumenMonedaCierre` y al motor de cierre | Obliga a tocar `prisma/schema.prisma` (con migración), `src/lib/cierre/**` y `src/lib/movimiento/caja.ts`: tres archivos de otros dueños y de un feature ya cerrado. F-034 es un feature de explicación en pantalla, y esto lo convertiría en una segunda vuelta a F-030 |
...
| Prorratear el crédito entre monedas por el peso de cada una en la caja | Inventa un dato: el crédito no tiene moneda física, por eso no es una línea de `pagosDetalle` (ADR 0104). Un número plausible y falso es peor que ninguno |
...
- F-034 se queda entero dentro de su mapa de propiedad: ni migración, ni motor, ni una línea de
  `src/lib/**`. Nada que escalar a F-030.
- La regla es una función pura con una firma corta, así que se prueba sin base de datos y sin
  ...
  distinta caja.
- El escenario central de F-034 y la mayoría de los negocios reales tienen un solo eje monetario:
