# ADR 0121: La reversión de un abono se descuenta de la caja como un espejo en negativo

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-033 (panel de cuentas por cobrar y registro de cobros), **por delegación de F-030**

> Cierra la contradicción 1 que el `arch-guardian` reportó al entregar el contrato y que el ADR 0120
> se limitaba a hacer inerte. **Extiende la propiedad de F-033 a dos puntos concretos de dos
> archivos de F-030** —los dos que filtran `tipo: "ABONO"`— y fija el tratamiento **con signo** que
...
F-033 estrena ese escritor. Mientras el filtro no cambie, **un abono cobrado y revertido dentro del
...
(`src/schemas/pago.ts`, de F-029) declara `monto: z.number().positive()` y
...
`netCollectionRows`, en `src/lib/cuentasPorCobrar/cobrosNetos.ts` (archivo nuevo, de F-033):
...
bajan, y el arqueo por destino del criterio 5 de F-030 sigue cuadrando—. **Ni `buildResumenMonedas`
...
Es aritméticamente correcto —la ecuación de reconciliación del ADR 0105 sigue cumpliéndose, porque
el efectivo de ese período también bajó por el mismo importe— y es lo que de verdad pasó: ese
período devolvió dinero de una deuda cobrada antes. La pantalla de cierre (F-034) **no debe
...
`REVERSION_ABONO` **no tiene escritor hasta que F-033 se despliegue**, así que **no existe ni una
fila de ese tipo en ninguna base de datos**. Ningún período cerrado antes de F-033 puede contener
...
| Opción | Por qué no |
|--------|-----------|
| **Dejarlo como estaba (ADR 0120 sin enmienda): la fila es inerte y la deuda queda anotada** | Es lo que el arquitecto propuso y el humano rechazó, con razón: entrega a sabiendas una cifra de caja incorrecta, y el ADR 0114 documenta cómo terminan las deudas «para quien lo posea» — en F-031 se difirió una y volvió doce días después |
| **Ampliar el filtro a los dos tipos y nada más** | Es el arreglo aparentemente obvio y **duplica el error**: `buildResumenMonedas` y `valueAbonos` solo suman, así que la reversión entraría como un ingreso más y la caja pasaría de sobrar X a sobrar 2X. Es la razón exacta por la que el ADR 0120 eligió `pagosDetalle: null` |
| **Persistir la reversión con `pagosDetalle` de importes negativos** | `pagoLineaSchema.monto` es `.positive()` y `equivalenteBase` `.nonnegative()`: la fila no parsearía por su propio schema de lectura, que es de F-029 y no se toca. Y rompería la invariante de forma del dosier § 5 |
...
- El arreglo es **una función pura de F-033** más dos cambios de consulta: es testable sin base de
...
- **F-033 escribe en dos archivos de F-030, que está en `passes: true`.** Es la razón de este ADR y
  de que el alcance esté acotado a los **dos puntos que filtran `tipo: "ABONO"`** y a sus `select`.
  Nada más de esos archivos se toca. **El `qa` de F-033 tiene que re-ejecutar los criterios de
  F-030 que el contrato enumera en su § 12**: no es opcional, y sin eso el `passes: true` de F-030
  deja de significar lo que significaba.
- **`totalCobrosCredito` puede quedar negativo.** Es correcto y es nuevo. F-034, dueño de la
...
- **Un tercer archivo nuevo en `src/lib/cuentasPorCobrar/`.** `cobrosNetos.ts` no cabe en
  ...
  del **panel**—, y no puede vivir dentro de los archivos de F-030 sin ampliar la delegación a
  archivos enteros. Extiende el ADR 0116 con el mismo argumento y **necesita el visto bueno
...
- **Seguridad, y es lo que hay que mirar con más cuidado de todo este ADR:** la consulta ampliada
  **conserva intacta** la cláusula de tenant, `cuentaPorCobrar: { tiendaId }`, y el `include` de
  ...
  apunta **a la misma cuenta** (ADR 0116). Aun así, el criterio 13 de F-030 —el que siembra un
