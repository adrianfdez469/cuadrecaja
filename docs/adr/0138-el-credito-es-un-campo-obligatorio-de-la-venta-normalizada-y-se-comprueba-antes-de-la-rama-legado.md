# ADR 0138: El crédito es un campo **obligatorio** de la venta normalizada, y se comprueba **antes** de la rama legado
...
El ADR 0111 sacó el crédito de `pagosDetalle` y lo puso en su propia columna, `Venta.creditoBase`.
...
- Nombre `creditAmount`, que es el que el dosier del epic propuso y el que `src/app/pos/**` ya usa
  para el mismo concepto desde F-034. Un sinónimo nuevo sería una segunda palabra para un hecho que
  ya tiene la suya.
...
- **Leído, nunca derivado.** No se calcula como `netAmount − Σ pagos`. El caso que lo demuestra no
  es hipotético: un pedido de tienda online entregado a crédito (ADR 0137) tiene `creditoBase`
...
| Un tercer valor en `pagoLineaSchema.tipo` | Lo prohíbe el ADR 0111 con cuatro razones, y rompería el cuadre de caja del producto entero |
...
| Una fila de crédito por moneda de cobro de la venta | El motor no guarda ningún desglose por moneda del crédito, y `creditoBase` está en base. Repartirlo por `monedaCobro` afirmaría una precisión que el dato no tiene — el mismo argumento del ADR 0131 |
