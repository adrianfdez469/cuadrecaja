# ADR 0148: El rastro de sincronización visible se decide por `wasOffline`, y `syncAttempts` solo cuenta desde 2

**Estado:** aceptado — **su umbral está enmendado por el [ADR 0149](0149-la-fila-declara-si-su-contador-cuenta-fallos-y-el-umbral-sale-de-esa-marca.md)** (2026-09-11)
**Fecha:** 2026-09-10
**Feature:** F-048

> **El segundo hemistiquio del título —«y `syncAttempts` solo cuenta desde 2»— ya no es cierto sin
> condición.** Desde F-050 el umbral depende de si la fila declara qué cuenta su contador: `1`
...
y permiten auditarlo después»*. Tras F-047 ese rastro **no era visible en ninguna pantalla**:
...
F-048 los trae al navegador (criterio 4) y pide que el detalle de una venta sincronizada tarde lo
...
- **La distancia entre `frontendCreatedAt` y `createdAt` no discrimina.**
  `.agents/designs/F-047.md` ya lo verificó: el POS manda su `Date.now()` y Postgres sella
...
- El spec de F-048 proponía **`wasOffline` con `syncAttempts`** como la señal que sí discrimina. La
...
- **Los dos campos ausentes se leen como «no marcada»**, nunca como «desconocida»: siguen siendo
  opcionales en `IVenta` porque F-048 corrige **un solo** productor.
...
`.agents/designs/F-048.md` añadió al mismo módulo un **selector de razones**,
...
| Derivar de la distancia entre `frontendCreatedAt` y `createdAt`, con o sin umbral | Verificado en `.agents/designs/F-047.md`: es no nula en **toda** venta, también en línea. El ADR 0108 ya descartó los umbrales de desfase por convertir el criterio en función del fixture |
| Unificar antes las dos convenciones de conteo de `syncAttempts` en el POS | Cambia lo que se **escribe**, no lo que hay escrito: haría falta además un backfill de las filas ya guardadas, y F-048 excluye explícitamente corregir ninguna fila existente. Queda declarado como deuda, abajo |
| ~~Una columna nueva `syncedLate` calculada al insertar~~ · ~~`>= 1` en vez de `>= 2`~~ · ~~unificar las convenciones~~ | **Las tres las revisó el ADR 0149 por su nombre y dos de ellas se adoptaron en otra forma.** Ver la enmienda del 2026-09-11 |
...
- **Las dos convenciones de conteo de `syncAttempts` no se corrigen aquí**, ni en el código ni en
  ...
  lo escribiera, y por eso el contrato de F-048 advierte que **no se pinte como «N intentos»** sin
...
- **Mostrar el rastro no impide la manipulación del reloj que el ADR 0108 describe.** La deja
  ...
  detalle la señalan. F-048 hace cierto lo que el ADR 0108 ya afirmaba; no cierra la puerta que ese
...
- **Aislamiento:** los dos campos son columnas de la **misma fila `Venta`** que el `GET` ya devolvía
  ...
  petición. Por eso F-048 no invoca a `security-guardian`, y queda escrito para que conste que se
  decidió.
...
## Enmienda del 2026-09-11: el umbral deja de ser una constante (F-050, ADR 0149)
...
F-050 corrige el código: el camino en línea del POS web deja de mandar el literal `1` y manda, como
...
- **Tres de sus alternativas descartadas las revisó el ADR 0149 por su nombre**: «una columna nueva
  ...
  dos convenciones» (se hace, y es F-050).
- **Sobre el APK, este ADR afirmaba menos de lo que ahora se afirma.** Decía: *«lo que ese cliente
  decida marcar es suyo, y esta decisión no lo puede afirmar»*. El criterio 3 de F-050, congelado,
...
y por tanto no enciende la sección»*. A partir de F-050, una venta **nueva** en esa situación **sí**
...
la decisión de este ADR de **no pintarla** se venía cumpliendo ahí **por accidente**. F-050 borra el
