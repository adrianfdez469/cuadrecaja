# ADR 0113: El nombre del cliente se rechaza en la puerta y se sanea en el ticket

**Estado:** aceptado
**Fecha:** 2026-09-09
**Feature:** F-032 (vender a crédito en el POS)

> Cierra el hallazgo **H1** de `.agents/F-032-seguridad.md`, que bloqueaba el paso 5. Enmienda el
> **ADR 0112**, cuya evaluación de riesgo no contemplaba esta consecuencia, y amplía la delegación
> escrita del **ADR 0111** para incluir `src/schemas/cliente.ts`.

## Contexto

F-032 hace dos cosas que, por separado, ya existían, y que juntas no habían existido nunca:

1. **Abre un segundo escritor de `Cliente.nombre`.** Hasta hoy el único era
   `POST /api/clientes`, detrás de `configuracion.clientes.acceder` (F-031, § 5.2). F-032 añade las
   dos rutas de venta, que **no exigen ningún permiso** (ADR 0112) porque la venta nunca exigió uno
   (ADR 0078).
...
Y `normalizeClienteNombre` (`src/lib/clientes/clienteNombre.ts`, de F-031, que F-032 reutiliza sin
...
| Refinar **solo** el schema, sin tocar el ticket | No cubre lo que ya está escrito —filas anteriores a la cota— ni lo que F-032 no gatea: `Usuario.nombre` llega al ticket por `Cajero: …`, los nombres de producto por los bloques, y ninguno de los dos tiene cota. El siguiente campo que alguien imprima nacería sin protección |
| Sanear en la interpolación de la línea `Cliente: …` únicamente | Cierra **una instancia** de la clase, no la clase. `left()`/`center()` son el sitio donde la cierra entera, y `buildTicketLines.ts` ya está en la lista de escritura de F-032: es el punto más barato al que este feature tiene derecho |
| Arreglar `encoder.ts`, que es donde de verdad está la deuda | No es un archivo de F-032 (contrato § 1) y arreglarlo bien significa decidir el escape de **todo** el flujo, incluidos los bytes `ESC` que el propio encoder emite a propósito para alinear, cortar y dibujar el QR. Es un trabajo con dueño propio, no algo que se hace de paso en un feature de crédito |
...
| Refinar también `clienteSchema` y `ventaSchema` (los modelos de lectura) | Haría que **leer** una fila ya almacenada lanzara: `clienteConSaldoSchema` (F-031) extiende `clienteSchema` y sus tests lo parsean, y un `reloadSales` dejaría de traer ventas por un dato incómodo. Un dato sucio en la base es un problema; una pantalla que no carga por él, uno peor |
...
- El escenario del hallazgo queda cerrado en los dos extremos: no entra por la puerta que F-032
...
- **Se edita un archivo más de un feature cerrado**, `src/schemas/cliente.ts` (F-031). Es una sola
  línea, restrictiva sobre una entrada, y ningún criterio de F-031 usa nombres con bytes de
  control, así que su `passes: true` se sostiene. Aun así es una edición sobre un contrato cerrado
  y queda anotada aquí y en el ADR 0111.
...
- **Aparece un estado de pantalla nuevo** que ningún criterio de aceptación de F-032 ejercita: el
...
- No hay impacto en el aislamiento multi-tenant: la cota es sobre la forma del dato, no sobre a
  quién pertenece. Todo lo que el ADR 0110 dice sobre `withTenantScope` sigue igual.
