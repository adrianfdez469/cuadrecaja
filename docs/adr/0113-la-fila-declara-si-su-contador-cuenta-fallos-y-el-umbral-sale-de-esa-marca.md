# ADR 0113: La fila declara si su contador cuenta fallos, y el umbral del rastro sale de esa marca

**Estado:** aceptado
**Fecha:** 2026-09-11
**Feature:** F-034
**Relación:** enmienda el ADR 0112 — **solo su umbral**. La señal principal de aquel documento
...
El ADR 0112 fijó que el rastro de sincronización visible se decide por `wasOffline`, y que
...
F-034 paga esa deuda. Y al pagarla, invalida el umbral.

### El diagnóstico cambió al verificarlo: no son dos convenciones, es una línea

Esto es lo primero que hay que saber, porque el ADR 0112 lo describe de otra manera. Leyendo los
tres productores:
...
Eso rompe el umbral del ADR 0112 por los dos lados a la vez, sobre la misma entrada
...
`hasSyncTrace` sigue siendo *«el selector devolvió algo»*, tal como fijó la enmienda del ADR 0112.
...
- **`/api/app` (APK): la sella el servidor**, sin condición. El APK no se toca y su convención la
  afirma el criterio 3 de F-034 como hecho verificado. Sellarlo aquí es lo que permite que **el
...
| **Bajar a `>= 1` para todo** | **Sobre-marca** casi toda venta en línea anterior al cambio, porque el `1` legado no es distinguible. Es el defecto que el ADR 0112 rechazó como `> 0` con otro nombre, y ahora además contradice el criterio 6 |
...
| **Que también el APK declare la marca en el cuerpo** | Cierra el hueco del punto 3 de *En contra*, pero exige tocar el contrato externo, que F-034 excluye. Queda anotado como la salida el día que ese contrato se revise |
...
### Tres filas del ADR 0112 que esta decisión revisa por su nombre

El ADR 0112 descartó estas tres opciones. Ninguna de las tres se contradice en silencio:
...
1. **«Una columna nueva calculada al insertar»** — rechazada por *«migración más backfill, fuera
   ...
   sentencia sin `UPDATE` y sin `DEFAULT`. Y el alcance es otro: F-032 excluía tocar filas
   existentes, F-034 tiene el criterio 6 exigiendo que se respeten. Además lo que se guarda no es
...
3. **«Unificar antes las dos convenciones de conteo»** — descartada entonces como deuda. **Esta
   decisión la paga**, y por eso existe F-034.
...
F-034, congelado y aprobado por el humano, **sí lo afirma** —con `sync_service.dart:497`, `:548` y
...
- **Una venta nueva con un solo reintento deja de ser invisible.** El ADR 0112 documentaba ese
...
- **La cifra sigue sin mostrarse en el historial web, y una comparación muerta se borra.**
  ...
  `saleSyncTrace.ts` — y eso el criterio 8 de F-034 no lo admite. **Se borra el bloque entero**, y
...
- **El único campo nuevo de entrada es un booleano que no participa en ninguna cláusula de
  ...
  tenant, y por eso F-034 no invoca a `security-guardian` — decidido, no olvidado.
