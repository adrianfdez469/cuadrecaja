# ADR 0145: `zoneTariffPrecedence` se reescribe entero a la forma de la v13, y el nivel se lee del campo declarado — no del `provinceCode`

**Estado:** aceptado
**Fecha:** 2026-09-11
**Feature:** F-041 (catálogo geográfico y función de precedencia)
**Reemplaza la implementación de:** F-026 (deprecated), commit `a9df077`
**Se apoya en:** [ADR 0143](0143-el-catalogo-de-zonas-vive-en-constants-y-su-hash-sale-del-disco.md) ·
...
exactamente la deducción por forma que el criterio 2 de F-041 existe para prohibir, y que el caso
...
5. **`resolveCoverage` no se reescribe: se retira.** No tiene consumidores, no lo pide ningún
   criterio de F-041, y especificarlo obligaría a probarlo. Cuando F-043 o F-044 necesiten resolver
...
| Conservar `resolveCoverage` adaptándolo | Un símbolo exportado que ningún criterio pide y que, si se declara, el `dev-tester` tiene que probar (E-035). Su forma útil depende de la pantalla de F-043, que aún no existe: congelarla ahora es adivinar |
...
- **Multi-tenant:** la función es pura y no consulta nada; el aislamiento por `negocioId` no entra en
  ella. Entra en F-043, que es quien persistirá las filas de tarifario por tienda y quien tendrá que
...
- **Escalabilidad:** el coste es lineal en el número de filas recibidas y el camino tiene como mucho
  dos escalones. F-043, cuando resuelva la cobertura de 168 municipios de una tienda, hará 168
