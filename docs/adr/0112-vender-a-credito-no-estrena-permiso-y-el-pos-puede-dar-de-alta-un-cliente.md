# ADR 0112: Vender a crédito no estrena permiso, y el POS puede dar de alta un cliente sin `configuracion.clientes.acceder`
...
F-032 les añade dos capacidades:
...
—`POST /api/clientes`— exige `configuracion.clientes.acceder` (F-031, § 5.2). Si la ruta de venta
...
- El **ADR 0109** reparte `configuracion.clientes.acceder` a `vendedor` y a `administrador`, y **no**
...
- Y F-031, § 5.1, ya dejó escrito —tras la auditoría del `security-guardian`— que **el saldo por
...
- Se reescribe el `motivo` de las **dos filas `POST`** de `routeGuards.json` para que diga que el
  ...
  compara los pares (ruta, verbo) y no cambia: F-032 **no añade ninguna fila**.
...
- **Un usuario sin `configuracion.clientes.acceder` puede hacer que nazca una fila de `Cliente`.**
  ...
  > **Enmienda del 2026-09-09** (hallazgo H1 de `.agents/F-032-seguridad.md`). Esa frase evaluaba
  > la consecuencia **de menos**. En la misma operación en que F-032 abre este escritor sin permiso,
  ...
  > abajo se sostienen enteras—, pero deja de sostenerse sola: el riesgo lo cierra el **ADR 0113**,
...
- La `CuentaPorCobrar` hereda su `tiendaId` de la misma variable ya persistida como `Venta.tiendaId`
  (ADR 0110): es la única vía por la que `TENANT_RELATION_PATH.cuentaPorCobrar` llega a `negocioId`.
- **Este ADR tuvo su segunda lectura del `security-guardian`** (`.agents/F-032-seguridad.md`,
  ...
  entidad nueva tiene que verse escrito y no deducirse del diff. Ver **ADR 0113**.
