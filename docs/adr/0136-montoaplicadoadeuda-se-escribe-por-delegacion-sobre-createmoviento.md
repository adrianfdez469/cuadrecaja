# ADR 0136: `montoAplicadoADeuda` se escribe por delegación sobre `CreateMoviento`
...
`MovimientoStock.montoAplicadoADeuda` (`prisma/schema.prisma:719`) existe desde F-031/F-032, y el
...
Mientras siga así, los criterios 9 y 10 de F-037 son inalcanzables.
...
**La ruta de devolución de F-037 es quien escribe la columna, y para llegar hasta ella se edita
`IMovimientoCreate`/`CreateMoviento` por delegación escrita a F-037.** Dos añadidos, los dos
...
(ADR 0121), y el precedente concreto es F-034 editando dos archivos de F-031 por delegación escrita
(ADR 0118).

**Quién decide el reparto no cambia:** `splitRefundBetweenDebtAndCash`
(`src/lib/cuentasPorCobrar/refundSplit.ts`, F-031) es la única función que lo decide, y este ADR no
...
- **`Venta.creditoBase` no se toca en una devolución.** La devolución no modifica el total de la
  ...
  libro. Es la diferencia con el borrado de un producto (ADR 0135), donde el total sí baja.
...
| Un helper propio de F-037 que envuelva `CreateMoviento` y haga el `update` después | Dos escrituras donde cabe una, y una ventana en la que la fila existe sin su reparto |
| Esperar a que un feature futuro se declare dueño de `src/lib/movimiento/**` | Bloquea F-037 indefinidamente por una cuestión de reparto, y F-037 tiene que ir en el mismo tren de despliegue que F-034 |
...
- Dos archivos sin dueño quedan editados por F-037. El contrato los nombra y dice exactamente qué
