# ADR 0143: El catálogo de zonas vive en `src/constants/`, se importa como JSON y su hash sale de leer el fichero — nunca de reserializar lo importado

**Estado:** aceptado
**Fecha:** 2026-09-11
**Feature:** F-041 (catálogo geográfico y función de precedencia)
**Se apoya en:** [ADR 0142](0142-la-copia-del-vector-se-commitea-como-markdown-con-su-valla.md) ·
[ADR 0144](0144-gitattributes-fija-eol-lf-en-los-dos-ficheros-fijados-por-hash.md) ·
[E-015](../../.agents/errors/E-015-un-simbolo-en-un-tsx-no-es-importable-desde-un-test.md)

## Contexto

F-041 siembra en este repositorio el artefacto de 184 zonas que publica QAB, byte a byte:
...
1. **Dónde vive el fichero.** `src/constants/` tiene el precedente exacto
   ...
   servidor** — y el consumidor natural del catálogo en F-044 es un selector de municipios que el
...
| Opción | Por qué no |
|--------|-----------|
| El fichero y el cargador en `src/lib/zones/` | `src/lib/` es lógica de servidor por convención de `AGENTS.md`. El catálogo es **dato**, y su consumidor de F-044 es una pantalla de selección que corre en el navegador. Meterlo en `src/lib/` invita a que alguien arrastre al cliente un módulo que la convención declara de servidor |
...
- `src/constants/zones/` queda como sitio evidente para la geometría de F-044, si llega.
...
- **Multi-tenant: ninguno.** El catálogo es geografía pública, idéntica para todos los negocios; no
  ...
  datos participa en este feature. El aislamiento entra en escena en F-043, cuando el tarifario
...
- **Escalabilidad:** 184 filas se cargan una vez por proceso. Las búsquedas por código van sobre un
  ...
  porque F-044 resolverá la cobertura zona a zona sobre la lista entera.
