# F-029 — Auditoría de seguridad del contrato (previa a implementación)

> Escrito por el agente `security-guardian`, paso 4 del pipeline, en paralelo con `arch-guardian`.
> No toca `.agents/specs/F-029.md`, `docs/adr/` ni código. **El código de F-029 todavía no existe:**
> esta auditoría es sobre el contrato de interfaces (`.agents/specs/F-029.md`, líneas 268-1486) y el
> ADR 0104, no sobre una implementación.

## Alcance auditado

- `.agents/specs/F-029.md` completo — spec (líneas 1-266) y contrato de interfaces (líneas 268-1486,
  §0-12).
- `.agents/cuentas-por-cobrar.md` — dosier del epic completo (§1-11).
- `docs/adr/0104-el-credito-es-una-columna-no-una-linea-de-pago.md` — completo.
...
Todo lo citado abajo se leyó, no se dedujo. F-029 no ejecuta ninguna consulta a la base de datos —
lo confirma el propio contrato en § 6.3 — así que esta auditoría es sobre **si el diseño del schema
y de las funciones puras deja abierta una puerta que un feature futuro (F-030 a F-037) heredaría sin
...
Ninguno impide que el `implementer` empiece a escribir código de F-029 tal como está descrito — no
...
**antes** de fijarse, porque F-029 es la única vez que alguien va a estar mirando estos dos campos
con lupa de tenant — el siguiente feature que los toque (F-032) va a heredar el comentario tal cual
...
**Dónde:** contrato § 2.2 (`.agents/specs/F-029.md:396-400`).
...
simple), así que la garantía completa depende de que quien cree la fila (F-032, en la misma
...
sitio para `CuentaPorCobrar.tiendaId` — ni en el contrato de F-029, ni en el dosier § 6, que solo dice
"crear la `CuentaPorCobrar` en la misma `$transaction` que la venta" sin decir de dónde sale
`tiendaId`.

**Por qué importa de verdad:** si F-032 tomara `tiendaId` de, por ejemplo, un parámetro de ruta
...
por escrito **en el contrato de F-029**, no dejarla implícita para que F-032 la invente sola.
...
/// schema enforces this — there is no composite FK for it — so the writer (F-032, which
...
**Dónde:** contrato § 2.3 (`.agents/specs/F-029.md:479-483`) y § 9 (firma reservada,
`.agents/specs/F-029.md:1330-1387`).

```prisma
/// A REVERSION_ABONO points at the ABONO it reverses. NULL for every other tipo.
/// F-029 creates this column and writes nothing into it; F-033 is its first writer.
...
F-029 no implementa el escritor (§ 9 lo reserva explícitamente para F-032 o F-033), así que hoy esto
...
## 🟠 Alta severidad — no bloquean el paso 5 de F-029, pero hay que fijarlos por escrito antes de que F-031/F-033 empiecen
...
choque de `@@unique([nombre, negocioId])` al reactivar un nombre (§ 2.1, `.agents/specs/F-029.md:373-380`),
...
alguna de sus cuentas. Si el panel de F-033 o el buscador de F-031 filtran clientes por
`deletedAt: null` — el patrón estándar de este mismo repo para `Producto`/`ProductoTienda`, citado
como precedente en el propio spec (`.agents/specs/F-029.md:148-151`) — un cliente con una deuda viva
...
**Por qué no bloquea F-029 específicamente:** ninguno de los doce criterios de aceptación de F-029
lo ejercita, F-029 no escribe `api/clientes/**` (es de F-031, dosier § 9) y no hay código de F-029
que dependa de la respuesta. El modelo `Cliente` tal como está definido en § 2.1 no necesita cambiar
para que F-029 pase sus doce criterios.

**Por qué sí hay que fijarlo ahora, no después:** es la única vez que alguien va a estar diseñando
el modelo `Cliente` con este contexto completo delante. Si se deja para que F-031 lo descubra al
escribir el DELETE, hay dos desenlaces malos: (a) F-031 decide una regla sin que el resto del epic
la conozca, o (b) F-031 no lo piensa y simplemente permite el soft delete sin guarda, y el agujero
...
- **Opción estricta:** el DELETE de `api/clientes/**` (F-031) devuelve 409 si el cliente tiene
  alguna `CuentaPorCobrar` con `settledAt IS NULL`. Mismo molde que el 409 que F-035 ya va a usar
  para "no se puede borrar una venta con abonos" (dosier § 6, tabla "Rutas de venta").
- **Opción pragmática:** permitir el soft delete igual, pero el panel de cuentas por cobrar (F-033)
...
**Dónde escribirlo:** no es una sección de F-029 propiamente (F-029 no implementa ningún gate), pero
...
la misma forma: qué pasa, y a quién le toca (F-031 o F-033, según cuál de las dos opciones elija el
humano). Alternativamente, en el dosier § 5 (Vocabulario), en la fila de `CuentaPorCobrar`.

---

## 🟡 Media severidad — el implementer de F-029 debe conocerlas; recomiendo fijarlas en el contrato porque es barato hacerlo ahora
...
| **Contrato F-029 § 3.2** `clienteSchema`/`createClienteSchema` | `nombre`, `descripcion`, `direccion`, `telefono` | **ninguna** |
| **Contrato F-029 § 3.3** `movimientoCuentaPorCobrarSchema` | `motivo` | **ninguna** |
...
### M2 — `checkCreditInvariant` es más permisivo que la frontera Zod que lo va a alimentar, y hay que dejarlo escrito para que F-032 no se confíe
...
Pero el propio contrato (§ 5.4, `.agents/specs/F-029.md:1117-1118`) fija que
...
nunca recibe hoy, ni recibirá en F-029, un `creditoBase` que no haya pasado ya por
...
### M3 — `IdempotencyKey` es global por `key`, y el dosier no advierte del patrón exacto de E-043 para cuando F-033 la use

**Contexto:** el contrato (§ 2.3, `.agents/specs/F-029.md:500-502`) dirige correctamente a F-033
...
columna de idempotencia `@unique`/`@id` global es un eje de tenant más. Si el escritor futuro (F-033)
...
**No es un hallazgo de F-029**: la tabla `IdempotencyKey` ya existe, F-029 no la toca ni la crea, y
...
segunda instancia del mismo patrón. Como el propio contrato de F-029 (§ 2.3) es quien introduce a
F-033 a esta tabla por primera vez en el epic, es el sitio natural para dejar la advertencia escrita,
en vez de confiar en que F-033 relea E-043 y haga la conexión sola.
...
No es una clase de riesgo nueva que F-029 introduzca: `MovimientoStock.motivo` y
...
### I3 — E-049 (deducir una clasificación de la forma de un identificador): no se encontró ninguna instancia en F-029
...
2. **La validación "el `clienteId` es del negocio de la venta" está explícitamente asignada, no
   ...
   dice literalmente "esa mitad... la hace F-032, no esta función" y el ADR 0104 la repite en su
   sección de "Autorización". El contrato de F-029 no cierra la puerta a que F-032 la implemente —
   al contrario, `checkCreditInvariant` deja `CREDIT_WITHOUT_CUSTOMER` como el primer chequeo,
   dejando sitio para que F-032 la complete con la consulta a la base.
...
4. **F-029 no añade ninguna ruta ni consulta a la base de datos** (§ 6.3, § 8) — confirmado: no hay
...
5. **El aparato de test que va a heredar el criterio 11** (`src/__tests__/fixtures/threeTenants.ts`,
   ...
   sin necesitar aún filas reales, ya que F-029 no tiene rutas que ejercitar contra la simulación
   completa de homónimo/control — eso les tocará a F-031/032/033 cuando añadan sus propias rutas al
   inventario.
...
alcanzable hoy — F-029 no ejecuta ninguna consulta, así que no hay superficie que atacar todavía.
...
son garantías de aislamiento multi-tenant que el próximo feature (F-032) va a asumir por leer el
comentario que encuentre, no por releer esta auditoría.

El hallazgo A1 (soft delete de `Cliente` con deuda viva) no bloquea a F-029 pero **sí necesita una
decisión del humano o del `arch-guardian` antes de que F-031 escriba el DELETE de `api/clientes/**`** —
recomiendo resolverlo en esta misma vuelta de contrato, ya que el modelo `Cliente` que lo origina es
de F-029, aunque quien lo aplique sea otro feature.
