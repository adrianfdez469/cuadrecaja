# F-030 — Auditoría de seguridad del contrato (previa a implementación)

> Escrito por el agente `security-guardian`, paso 4 del pipeline. No toca
> `.agents/specs/F-030.md`, `docs/adr/` ni código. **El código de F-030 todavía no existe:**
> esta auditoría es sobre el contrato de interfaces (`.agents/specs/F-030.md`, spec líneas
> 1-334 y contrato §0-14, líneas 337-1387) y el ADR 0105, no sobre una implementación.

## Alcance auditado

- `.agents/specs/F-030.md` completo — spec y contrato de interfaces §0-14.
- `.agents/cuentas-por-cobrar.md` — dosier completo, con foco en §3, §6, §7 y §8.
- `docs/adr/0105-la-ecuacion-de-reconciliacion-de-caja.md` completo.
- `.agents/specs/F-029.md` §6 (vía `src/constants/tenantScope.ts` y el propio contrato de
  F-030 §10, que la cita) y `.agents/F-029-seguridad.md` completo.
...
- `prisma/schema.prisma`: `Cliente` (1383-1403), `CuentaPorCobrar` (1407-1459),
  ...
  `.agents/F-029-seguridad.md` pidió (B1, B2) están de verdad escritas en el schema y no solo
  prometidas.
...
- `src/constants/routeGuards/routeGuards.json` (entradas de `cierre/**`) — para confirmar el
  estado de autorización ya aceptado de las rutas que F-030 toca.
...
introducida por F-030, y no es una decisión de producto que este informe pueda reabrir.

**Lo que sí es nuevo y lo que este informe pide:** el contrato de F-030 (§1.1, §6.2, §3.1)
...
usuario", pero el contrato de F-030 no llega a decidir si el saldo por cobrar y el crédito
...
eventualmente oculto en la UI que construya F-034) es consistente con cómo se trata
`totalGanancia` hoy, y no introduce una clase de vulnerabilidad nueva — solo la extiende.
Pero es exactamente el tipo de vacío que `.agents/F-029-seguridad.md` (B1, B2) ya señaló que
hay que cerrar por escrito la primera vez que alguien mira el archivo con lupa, no dejar que
lo decida F-034 por su cuenta mientras diseña la pantalla.
...
acceso al cierre, y es la pantalla (F-034) quien decide si las oculta detrás de
...
sensibilidad que el margen, y F-034 DEBE gatearlas en la pantalla detrás de
operaciones.cierre.gananciascostos cuando construya la vista. F-030 no cambia el backend
porque no le corresponde (F-030 no toca ninguna pantalla), pero deja escrita la obligación
para que F-034 no la decida sin este contexto.
```
...
2. **El §11 nunca dice qué se borra al terminar.** El §11.1 solo fija que el **script** se
   ...
   verificación de F-031/F-032 que reutilice la misma tienda) pueden encontrar cuentas y
...
el ADR 0105 («Reversibilidad») explican qué pasa si se **revierte el feature entero**
...
recalcular pueda pasar por alto (F-030 no toca `applyMovimientoCuentaPorCobrar`, así que no
hay caché a resincronizar aparte). Pero esto **no está escrito en ningún sitio del contrato
de F-030**, y es exactamente la clase de nota que `.agents/F-029-seguridad.md` (B1/B2) dejó
para que el siguiente feature no la reinventara — aquí es al revés: F-030 es quien introduce
...
**Cambio concreto pedido al `arch-guardian`, en el contrato § 10** (o en el ADR 0105, sección
...
otra ruta que F-030 modifica (§6.1: gana `cobrosCreditoEfectivo` en cada fila). Como las tres
...
3. **F-030 no escribe en `MovimientoCuentaPorCobrar` ni en `CuentaPorCobrar`** (§0.1 del
...
4. **F-030 no añade ni cambia ninguna ruta** (§6.1): cero entradas nuevas en
...
5. **Las anotaciones de invariante que `.agents/F-029-seguridad.md` pidió (B1, B2) están
   ...
   EXISTS... The check lives in applyMovimientoCuentaPorCobrar`. El contrato de F-030 §10
...
protegida por un criterio que la ejercita con un control positivo, y F-030 no abre ningún
...
- **H1** — que el `arch-guardian` decida y escriba, en § 6.2, si las tres cifras de crédito
  ...
  deben quedar reservadas para cuando F-034 las gatee explícitamente.
...
- **H3** — una frase en §10 (o en el ADR 0105) sobre cómo se repara `totalPorCobrarAlCierre`
...
tres caben como texto adicional, igual que las anotaciones que `.agents/F-029-seguridad.md`
pidió para F-029. M1 y M2 son observaciones de cobertura, no exigen ni un criterio nuevo ni un
cambio de contrato.
