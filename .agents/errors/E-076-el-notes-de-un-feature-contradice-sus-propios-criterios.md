# E-076: El `notes` de un feature contradice sus propios `acceptance_criteria`

**Área:** build
**Apariciones:** 1 — F-035 (paso 4, detectado por el `arch-guardian`)
...
En F-035, el `notes` prescribía la solución:
...
En F-035, el `arch-guardian` lo detectó al cerrar el contrato y **no eligió en silencio**: lo
...
Quedó fijada en el ADR 0126, y el criterio 5 se siembra con un **abono parcial**, que sigue siendo
...
quedan inalcanzables bajo cada lectura. Es información que el humano no tiene delante, y en F-035
...
el `spec` ya escribió para los criterios afectados queda obsoleto**. En F-035 el del criterio 5
