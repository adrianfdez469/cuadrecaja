# E-080: El `tsc` de árbol completo tampoco es señal limpia durante el paso 5

**Área:** build
**Apariciones:** 1 — F-039 (`implementer`, reescribiendo `zoneTariffPrecedence.ts`)
...
En F-039 el § 1 del contrato mandaba reescribir una pareja heredada de F-026:
...
válida entre tanto.** El § 1 de F-039 asignó correctamente cada fichero a su agente, y aun así no
...
- Para el coordinador: un `tsc` en rojo a mitad del paso 5 no es motivo para interrumpir a nadie.
  La comprobación que vale es la de después, con los dos agentes aterrizados — que en F-039 dio
  exit 0 limpio.
