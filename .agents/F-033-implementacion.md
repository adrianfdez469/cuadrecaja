# F-033 — Informe de implementación (`implementer`)

**Fecha:** 2026-09-10
**Contrato:** `.agents/specs/F-033.md` §§ 370-1858 (versión enmendada del 2026-09-09)
**Diseño:** `.agents/designs/F-033.md` (versión actualizada tras la enmienda)
...
literalmente cambiar ese `where` a `tipo: { in: ["ABONO", "REVERSION_ABONO"] }` (ADR 0121). El test
es preexistente, de F-030, y **vive en la frontera del `dev-tester`**: no lo edito (E-046). Hay que
...
| `src/lib/cuentasPorCobrar/cobrosNetos.ts` | `netCollectionRows`: el espejo en negativo del ADR 0121 |
...
2. **El bloqueo y el importe del perdón (contradicción 6 de arriba).** Tardé dos vueltas en ver que
   ...
   **inalcanzable** si la ruta pasa el saldo leído en el paso 4: con 500 obsoleto sobre un saldo
   ...
   de leer bajo el bloqueo desde fuera de la puerta. Es la figura inversa de **E-064**: el valor de
...
Los dos bloqueantes del § 10 de `.agents/F-033-qa.md`, más la discrepancia de localizadores del
criterio 47(b).
...
propio servidor no satisface ese schema**: `mapVentaToIVenta` (`src/lib/ventaMapper.ts`, F-035)
...
`ventaMapper.ts` ni `src/schemas/usuario.ts` son míos (§ 11 del contrato), así que F-033 estrecha
**lo que él parsea** en vez de tocar ninguno de los dos.

> **DEUDA DECLARADA, sin dueño en F-033:** `mapVentaToIVenta` produce un `IVenta` que
> `ventaSchema` rechaza. Es hermana de la deuda 1 que ya declaré (el mapper no copia `creditoBase`
> ni `clienteId`) y **es de F-035**. Mientras siga así, ningún consumidor puede validar una venta
...
**Dentro de F-033: no queda ninguno.** Barrí los consumidores de métodos de `Date`
...
**Fuera de F-033: el patrón es del repositorio entero, y no lo toco.** De ~34 archivos en
...
material para una ficha de error; no es de F-033 arreglarlo.
...
1. **El arreglo obvio del bloqueante 1 era el fallo peor.** `deudorDetalleResponseSchema.parse(...)`
   ...
   y ninguno de los dos documentos dice que el mapper de F-035 no satisface el schema que F-033
   declara para su propia respuesta. Es **E-064 al revés**: no un valor de ejemplo que contradice
...
**NO en el interceptor, NO en el servicio: en mi página.** Es un bug de F-033, no de media
aplicación.
