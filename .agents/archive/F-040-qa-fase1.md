# F-040 — QA fase 1: líneas base (antes del cambio de código)
...
comportamiento de `claimOutboxBatch`/`intentos<6` es correcto y preexistente, ajeno a F-040), pero
...
- Sondas usadas: `qaf038-sql.ts` (SQL de lectura vía Prisma) y `qaf038-probe.ts` (el probe § 3 del
  ...
  preexistentes de otros agentes de este mismo feature (`.agents/progress/F-040.md`,
  `.agents/security/F-040.md`, `.agents/specs/F-040.md`), que no son míos.
- `grep -rnE '(/Users/|/home/|~/|[A-Z]:\\)' .agents/ .claude/ --include='*.md' --include='*.json'`
  ...
  F-040 (`spec`, `security`, `progress`) están limpios.
...
> alcanzable. Sigue sin tocarse `src/`, tests, `features.json` ni `.agents/specs/F-040.md`; el
