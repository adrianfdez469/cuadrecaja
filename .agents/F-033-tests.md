...
| `src/__tests__/cuentasPorCobrarCobrosNetos.test.ts` | 8 | 15: `netCollectionRows` (el espejo en negativo, ADR 0121) |
...
Más un arreglo sobre un archivo **preexistente de F-030** que el contrato de F-033 rompe desde
fuera (ver «Arreglos pedidos por el coordinador»):

| Archivo | Qué cambió |
|---|---|
| `src/__tests__/loadCierreInput.test.ts` | Las dos aserciones de `periodCollectionsWhere` actualizadas de `tipo: "ABONO"` a `tipo: { in: ["ABONO", "REVERSION_ABONO"] }` (contrato § 12.1, ADR 0121). Un tercer test nuevo confirma explícitamente que no se relajó a un `.not.toBe("ABONO")` sin más. |

No toqué `src/__tests__/routeGuardInventory.test.ts`: es genérico y data-driven (lee
`routeGuards.json` y el árbol en disco y los compara), así que las seis rutas de F-033 lo
...
ya existe, exacto y ya en verde, en `src/__tests__/cuentasPorCobrarSaldo.test.ts` (F-029, líneas
...
De paso, el propio contrato (`.agents/specs/F-033.md` § 4, actualizado por el `arch-guardian`) fijó
...
código.** El § 12.1 (ADR 0121) amplía el `where` de `periodCollectionsWhere` de `tipo: "ABONO"` a
...
— E-068), lo corregí generando UUIDs reales, y en la segunda corrida las 8 ya estaban en verde
...
| — (F-030 § 12.1, protegido desde fuera) | `periodCollectionsWhere` incluye `REVERSION_ABONO` y no se reduce de vuelta a `"ABONO"` | `loadCierreInput.test.ts` |
...
delegados de F-030. No leí ninguno de esos archivos con la herramienta de lectura, ni mientras se
...
- **La segunda mitad de § 12.1** (el `select`/`revierte` de `loadCierreInput`/`caja.ts`): ver
  ...
  contrato.
- **Los 10 criterios de F-030 que el § 12.2 del contrato exige re-ejecutar**: integración con
...
- Confirmado ejecutando: `src/__tests__/routeGuardInventory.test.ts` **no necesita ningún cambio**
  para F-033 — es genérico. Si en algún punto del pipeline se sigue repitiendo la instrucción de
