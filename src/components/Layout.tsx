...
const MAIN_MENU_ITEMS = [
  ...
  },
  {
    ...
    path: "/conformar_precios",
    icon: GridView,
    permission: "operaciones.conformarprecios.acceder",
  },
  {
    label: "Gastos",
    path: "/gastos",
    icon: TrendingDown,
    permission: "operaciones.gastos.ver",
  },
  {
    label: "Cierre",
    path: "/cierre",
...
const RESUMEN_MENU_ITEMS = [
  {
    ...
    icon: Summarize,
    permission: "recuperaciones.dashboard.acceder",
  },
  {
    label: "Resumen Cierres",
    path: "/resumen_cierre",
    icon: Summarize,
    permission: "recuperaciones.resumencierres.acceder",
  },
  {
    label: "Análisis de CPP",
    path: "/cpp-analysis",
    icon: Summarize,
    permission: "recuperaciones.analisiscpp.acceder",
  },
  {
    label: "Proveedores Consignación",
    path: "/proveedores",
    icon: Handshake,
...
const Layout: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const getMenuTourAttr = (path: string): string | undefined => {
    if (path === "/inventario") return "nav-gestion-inventario";
  ...
  const getMainMenuItemsByLocalType = (
    localType: string,
    type: string,
    user: { rol: string; permisos: string },
  ) => {
    let items = [];
    if (type === "resumen") {
  ...
  const handleCambiarLocal = async () => {
    try {
      ...
      console.error(error);
      showMessage(
        "No se pueden cargar los locales disponibles",
        "error",
        error,
      );
    } finally {
  ...
  const handlePromoterQuickAccess = async () => {
    ...
    try {
      ...
      if (!response.ok || !data?.ok) {
        showMessage(
          data?.error || "No se pudo abrir el panel de promotor.",
          "error",
        );
        return;
      }
      ...
      console.error(error);
      showMessage(
        "Error de conexión al habilitar el acceso de promotor.",
        "error",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    if (
      ...
      user &&
      !user.localActual &&
      totalLocalesDisponibles >= 1 &&
      !openSelectLocal &&
      !cambiandoNegocio &&
      !selectorLocalAbiertoRef.current
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      new Date() > new Date(session.user.expiresAt)
    ) {
      signOut();
    }
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
          ...
          {isAuth && user ? (
            ...
            >
              ...
              >
                ...
                <MenuItem
                  onClick={handlePromoterQuickAccess}
                  disabled={loadingPromoterAccess}
                >
                ...
                {(user.rol === "SUPER_ADMIN" ||
                  totalLocalesDisponibles > 1 ||
                  (totalLocalesDisponibles >= 1 && !user?.localActual)) && [
                  <MenuItem
                    key="cambiar-local"
                    onClick={() => handleCambiarLocal()}
                  >
      ...
      >
        ...
        >
          <Suspense>{isNavigating ? <Loading /> : children}</Suspense>
        </Container>
      </Box>
          ...
          {localesDisponibles.length === 0 && !loadingLocales && (
            ...
            >
              ...
              >
                ...
                administrador para crear locales.
              </Typography>
            </Box>
          )}
          {loadingLocales ? (
            ...
            <RadioGroup onChange={(e) => handleSelectLocal(e.target.value)}>
              {localesDisponibles
                ?.filter((local) => local.id !== user?.localActual?.id)
                .map((local) => (
                  <FormControlLabel
                    key={local.id}
                    value={local.id}
        ...
        >
          ...
          <DialogContent sx={{ pt: 2 }}>
            {loadingNegocios ? (
              ...
              <RadioGroup onChange={(e) => handleSelectNegocio(e.target.value)}>
                {negocios
                  .filter((negocio) => negocio.id !== user?.negocio?.id)
                  .map((negocio) => (
                    <FormControlLabel
                      key={negocio.id}
                      value={negocio.id}
