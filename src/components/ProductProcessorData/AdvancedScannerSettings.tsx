 ...
 */
export const AdvancedScannerSettings: React.FC<
  AdvancedScannerSettingsProps
> = ({ open, onClose, onApply, currentOptions }) => {
  const [fps, setFps] = useState(currentOptions.fps || 10);
  const [width, setWidth] = useState(currentOptions.resolution?.width || 1280);
  const [height, setHeight] = useState(
    currentOptions.resolution?.height || 720,
  );
    ...
    >
      ...
      <DialogContent>
        <Alert severity="info" sx={{ mb: 3 }}>
          Ajusta estos valores solo si experimentas problemas de rendimiento o
          calidad.
        </Alert>
