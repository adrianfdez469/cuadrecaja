      ...
      <DialogContent sx={{ pb: 1, px: isMobile ? 1 : 3 }}>
        <Stack spacing={3}>
          ...
          {isMobile ? (
            ...
            <Stack spacing={2}>
              {planes.map((plan) => {
                const price = plan.precio === -1 ? 'Cotización' : plan.precio === 0 ? '$0' : `$${plan.precio}`;
                const period = plan.precio === 0 || plan.precio === -1 ? '' : 'mes';
                      ...
                      <Stack spacing={2}>
                        <Box>
                          ...
                          <Box sx={{ mt: 1, display: 'flex', alignItems: 'baseline', gap: 0.5 }}>
                            <Typography variant="h5" component="span" color="primary" sx={{ fontSize: '1.5rem', fontWeight: 700 }}>
                              {price}
                            </Typography>
                            {period && (
                    ...
                    >
                      {plan.recomendado && (
                        <Chip
                          ...
                          size="small"
                          icon={<Star />}
                          sx={{ position: 'absolute', top: -10, left: '50%', transform: 'translateX(-50%)', zIndex: 1 }}
                        />
                      )}
                      <CardContent sx={{ p: isTablet ? 2 : 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
                        <Box sx={{ textAlign: 'center', mb: 2 }}>
                          ...
                          <Box sx={{ my: 2 }}>
                            <Typography variant="h4" component="span" color="primary" sx={{ fontSize: isTablet ? '1.75rem' : '2rem' }}>
                              {price}
                            </Typography>
                            {period && (
                              <Typography variant="body2" component="span" color="text.secondary">
