...
export function EditProductDialog({ open, producto, categorias, onClose, onSave }: Props) {
    ...
    if (esFraccion && productos.length === 0) {
      fetchProducts().then(prods => {
        ...
        if (producto?.producto.fraccionDeId) {
          const found = prods.find(p => p.id === producto.producto.fraccionDeId);
  ...
  const handleRemoveCodigo = (idx: number) => setCodigosProducto(prev => prev.filter((_, i) => i !== idx));
  const handleCodigoChange = (idx: number, val: string) => setCodigosProducto(prev => prev.map((c, i) => i === idx ? val : c));
            ...
            {codigosProducto.map((codigo, idx) => (
              <Box key={idx} display="flex" alignItems="center" mb={1} gap={1}>
                <HardwareQrScanner
                  ...
                  keepFocus={false}
                />
                <MobileQrScanner qrCodeSuccessCallback={qrText => handleCodigoChange(idx, qrText)} />
                <Tooltip title="Eliminar código">
                  <IconButton onClick={() => handleRemoveCodigo(idx)} size="small">
