...
function ActivarUsuarioForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(typeof data.error === "string" ? data.error : "No se pudo activar la cuenta.");
        return;
      }
      ...
      <Paper elevation={3} sx={{ borderRadius: 2, overflow: "hidden" }}>
        <CardContent sx={{ p: 4 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Tu usuario quedará activo al definir una contraseña segura (mayúsculas, minúsculas y
