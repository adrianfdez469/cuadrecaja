...
interface PricingSectionProps {
  /** Active plans, already read on the server by the landing page. */
  planes: IPlan[];
}
 ...
 *
 * The design draws four cards and a row, but the list is whatever the page
 * read on the server and is active: adding a plan in configuración has to show
 * up here without a deploy, which the landing's ISR window takes care of.
 * Plans priced `-1` are negotiated, so they drop out of the grid and into the
 * row underneath, which is the only shape that fits a plan with no number to
 * compare.
 *
 * This stays a client component for the billing-cycle toggle alone; the data
 * arrives as a prop precisely so the browser never asks the API for it.
 */
export function PricingSection({ planes }: Readonly<PricingSectionProps>) {
