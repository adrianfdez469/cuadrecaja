...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      (!parsed.data.monto || parsed.data.monto <= 0)
    ) {
      return NextResponse.json(
        {
          error:
            "El monto es requerido y debe ser mayor a 0 para esta plantilla",
        },
        { status: 400 },
      );
    }
    if (
      plantilla.tipoCalculo !== "MONTO_FIJO" &&
      (!parsed.data.porcentaje || parsed.data.porcentaje <= 0)
    ) {
      return NextResponse.json(
        {
          error:
            "El porcentaje es requerido y debe ser mayor a 0 para esta plantilla",
        },
        { status: 400 },
      );
    }
