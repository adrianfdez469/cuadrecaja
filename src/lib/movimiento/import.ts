...
const validarYSanitizarItem = (
  ...
  if (!item.nombreProducto || typeof item.nombreProducto !== "string") {
    ...
    if (nombreSanitizado.length === 0) {
      errores.push(
        "nombreProducto no puede estar vacío después de la sanitización",
      );
  ...
  if (item.esConsignación) {
    if (item.nombreProveedor) {
      ...
      if (proveedorSanitizado.length === 0) {
        errores.push(
          "nombreProveedor no puede estar vacío cuando esConsignación es true",
        );
  ...
  return {
    itemSanitizado: {
      ...
      nombreProveedor: item.nombreProveedor
        ? sanitizarString(item.nombreProveedor, 255)
        : undefined,
...
const procesarChunk = async (
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const item of items) {
      ...
      const categoriaId = await getCategoryId(
        nombreCategoria,
        data.negocioId,
        tx,
      );
      ...
      const productoTienda = await tx.productoTienda.create({
        data: {
          ...
          ...(item.monedaCostoCode && {
            monedaCostoCode: item.monedaCostoCode,
          }),
          ...(item.monedaPrecioCode && {
            monedaPrecioCode: item.monedaPrecioCode,
          }),
        },
      });
      ...
      await tx.movimientoStock.create({
        data: {
          ...
          ...(proveedorId && { proveedorId: proveedorId }),
          ...(item.monedaCostoCode && { monedaOriginal: item.monedaCostoCode }),
          montoOriginal: item.costo,
        },
      });
  ...
  try {
    ...
    if (itemsInvalidos.length > 0) {
      return {
        ...
        errorCause: `Items inválidos: ${itemsInvalidos
          .map(
            (item) =>
    ...
    const tiendaPerteneceAlNegocio = negocio.tiendas.some(
      (tienda) => tienda.id === data.localId,
    );
    ...
    const clavesDuplicadas = claves.filter(
      (clave, idx) => claves.indexOf(clave) !== idx,
    );
    ...
    if (error.code === "P2003") {
      return {
        ...
        errorCause:
          "Se intentó crear una referencia a un registro que no existe",
      };
    }
    ...
    if (error.code === "P2034") {
      return {
        ...
        errorCause:
          "La transacción falló debido a un conflicto de concurrencia",
      };
    }
...
const procesarLoteProductos = async (
  ...
  for (const nombreProducto of productosUnicos) {
    const nombreCategoria =
      items.find((p) => p.nombreProducto === nombreProducto)?.categoria ||
      "SIN CATEGORIA";
    const categoriaId = await getCategoryId(
      nombreCategoria,
      data.negocioId,
    ...
    try {
      ...
      if (existeProductoTienda) {
        errores.push({
          ...
          nombreProducto: item.nombreProducto,
          error:
            "Ya existe productoTienda para este producto y proveedor en la tienda",
