...
| # | Archivo | Nota |
|---|---------|------|
| P2.1 | `src/lib/movimiento/index.ts` → `CreateMoviento` (tx en ~67) | Loop por item con ~4-6 queries c/u. **Solo** la rama `esCompraEfectivoCaja` tiene `{ timeout: 15000, maxWait: 10000 }` (~línea 375); traspasos, mermas y ajustes corren con el default de 5000ms. |
